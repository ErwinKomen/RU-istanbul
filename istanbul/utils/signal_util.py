...
def catch_m2m(instance, action, pk_set, model_name,field_name):
	...
	if z:
		...
		# easyaudit does not catch m2m changes, here we update the attr with m2m 
		# changes
		event = CRUDEvent.objects.filter(content_type__model = model_name,
...
def make_m2mreceiver(sender,model_name,field_name):
	...
	m+= 'def '+sender.replace('.','__').lower() 
	m+='(sender,instance,action,**kwargs):\n'
...
def show(sender,instance,**kwargs):
	...
	print(m,pmn,smn)
	c = CRUDEvent.objects.filter(content_type__model = m.lower(),
		object_id=instance.pk)
	pe = CRUDEvent.objects.filter(content_type__model = pmn.lower(),
		object_id=instance.primary.pk)[0]
	se = CRUDEvent.objects.filter(content_type__model = smn.lower(),
		object_id=instance.secondary.pk)[0]
...
def extract_filename_and_path(name):
	...
	if '/' not in name: filename, remote_path = name, ''
		...
		filename, remote_path = name.split('/')[-1], '/'.join(name.split('/')[:-1])
	remote_path += '/'
...
# receivers to set person field on text, illustration, publisher, 
# periodical and movement
