    ...
    return {
      ...
      init_events: function (sUrlShow, options) {
        ...
        try {
          ...
          // Textarea vertical size
          $(".autosize textarea").on("input", function () {
          ...
          switch (method) {
            ...
            default:
              ...
              if (contentid !== undefined && contentid !== null && contentid !== "") {
                ...
                // Check if data needs to be downloaded locally
                elFormData = $(elStart).closest("table.formdata");
                if (elFormData !== null) {
                  // There may be more data to be added
                  $(elFormData).find("input").each(function (idx, el) {
                    var sName = "", sValue="";
                    sName = $(el).attr("name");
                    sValue = $(el).val();
                    // Add element
                    $(elData).append({ name: sName, value: sValue });
                  });
                }
                ...
                if (attempt === "xhr") {
                  ...
                  if (elFormData !== null) {
                    // There may be more data to be added
                    $(elFormData).find("input, select").each(function (idx, el) {
                      var sName = "", sValue = "";
                      sName = $(el).attr("name");
                      sValue = $(el).val();
                      // Add element
        ...
        try {
          ...
          if (elRoot.length > 0) {
            ...
            if (elOriginal.length > 0 && elCopy.length > 0) {
              ...
              $(elOriginal).find(".edit-mode").addClass("hidden");
              // switch on 'view-mode'
              $(elOriginal).find(".view-mode").removeClass("hidden");
      ...
      related_edit: function (elStart) {
        ...
        try {
          ...
          if (elRoot.length > 0) {
            elOriginal = $(elRoot).find(".related-original").first();
            // switch off 'view-mode'
            $(elOriginal).find(".view-mode").addClass("hidden");
            // switch on 'edit-mode'
            $(elOriginal).find(".edit-mode").removeClass("hidden");
          }

        } catch (ex) {
          private_methods.errMsg("related_edit", ex);
        }
      },

      /**
       * related_change
       *    User changed something, which means that 'save' must become visible
       *
       */
      related_change: function (elStart) {
        try {
          // Show that changes can/need to be saved
          $(elStart).closest("table").closest("div").find(".related-save").removeClass("hidden");

        } catch (ex) {
          private_methods.errMsg("related_change", ex);
        }
      },
      ...
      related_dragenter: function (ev) {
        ...
        try {
          ...
          if (ev.target.nodeName.toLowerCase() === "td" && $(ev.target).hasClass("draggable")) {
            ...
            $(ev.target).closest("table").closest("div").find(".related-cancel").removeClass("hidden");
      ...
      rel_row_edit: function (elStart, sAction, editcols) {
        ...
        try {
          ...
          // Get the target id
          targetid = $(elRow).find(".rel-edit-mode").first().closest("td");
          // Action switching
          switch (sAction) {
            case "open":    // Hide summary view and enter edit view
              ...
              $(elRow).find(".autosize textarea").each(function (idx, el) {
                el.style.height = "auto";
                el.style.height = el.scrollHeight + 10 + "px";
              });
              break;
            case "close":   // Hide edit view and enter summary view
              // Check if there is any form to send
              elForm = $(elRow).find("form").first();
              if (elForm === undefined || elForm === null) {
