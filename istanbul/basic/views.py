...
def get_previous_page(request, top=False):
    """Find the previous page (user-independant)"""
...
def action_model_changes(form, instance):
    ...
    for item in changed_fields: 
        if item in field_values:
            if isinstance(field_values[item], datetime):
...
def make_search_list(filters, oFields, search_list, qd, lstExclude, bUseContains = True):
                        ...
                        if has_string_value(keyId, oFields):
                            ...
                            for fkfield in arFkField:
                                if "*" in val or "#" in val:
                                    ...
                                    s_q_add = Q(**{"{}__{}__icontains".format(fkfield, keyFk): val})
                                else:
...
def add_rel_item(rel_item, value, resizable=False, title=None, align=None, link=None, nowrap=True, main=None, 
                 draggable=None, editable=None):
    ...
    if editable != None: oAdd['editable'] = editable
        ...
        try:
            ...
            if self.basketview:
                ...
                if thisForm.is_valid():
                    ...
                    self.filters, lstQ, self.initial, lstExclude = make_search_list(self.filters, oFields, self.searches, self.qd, lstExclude, self.use_contains)
                    ...
                    oErr.Status("get_queryset errors: {}".format(thisForm.errors))
...
class BasicDetails(DetailView):
    """Extension of the normal DetailView class for BTCHUB"""
    ...
    def get(self, request, pk=None, *args, **kwargs):
        ...
        if not regard_as_authenticated(request):
            ...
            elif self.object is None and self.redirectpage != "":
    ...
    def initializations(self, request, pk, **kwargs):
        ...
        self.custom_init(self.object, **kwargs)
...
class BasicPart(View):
        ...
        if self.checkAuthentication(request):
            ...
            if self.action == "":
                ...
                if self.dtype != "":
                    ...
                    # First try user's response
                    response = self.response_download()
                    ...
                    if self.dtype == "excel":
                        ...
                        extension = "xml"
                    sDateTime = get_current_datetime().strftime("%Y-%m-%d-%H%M%S")
                    sDbName = "btchub_{}_{}_{}.{}".format(modelname, obj_id, sDateTime, extension)
                    sContentType = ""
                    if self.dtype == "csv":
                        ...
                        sDbName = "btchub_{}_{}.svg".format(modelname, obj_id)
                        ...
                        if len(arPart) > 1:
                            ...
                            # Set the filename correctly
                            sDbName = "btchub_{}_{}.png".format(modelname, obj_id)
    ...
    def initializations(self, request, object_id, **kwargs):
        # Store the previous page
        #self.previous = get_previous_page(request)
        # Clear errors
        self.arErr = []
